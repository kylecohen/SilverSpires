...
namespace SilverSpires.Tactics.Export
{
    using System.Text;
    using System.Linq;
    using SilverSpires.Tactics.Core;
    ...
    {
        public static string CharacterSheet(PlayerCharacter pc)
        {
            var sb = new StringBuilder();

            sb.AppendLine($"Name: {pc.Name}");
            sb.AppendLine($"Level: {pc.Level}");
            sb.AppendLine($"Race: {pc.Race?.Name ?? "Unknown"}");
            sb.AppendLine($"Class: {pc.Class?.Name ?? "Unknown"} (Hit Die d{pc.Class?.HitDie ?? 0})");
            sb.AppendLine($"Background: {pc.Background?.Name ?? "Unknown"}");
            sb.AppendLine($"Alignment: {pc.Alignment}");
            sb.AppendLine();

            sb.AppendLine("Ability Scores:");
            foreach (var kvp in pc.AbilityScores.AsReadOnly())
            {
                var score = kvp.Value;
                sb.AppendLine($"  {score.Type}: {score.Score} (mod {score.Modifier:+#;-#;0})");
            }
            sb.AppendLine();

            sb.AppendLine($"Proficiency Bonus: {pc.ProficiencyBonus:+#;-#;0}");

            var hp = pc.Resources.FirstOrDefault(r => r.Type == ResourceType.HitPoints && r.Key == "HP");
            if (hp != null)
            {
                sb.AppendLine($"Hit Points: {hp.Current}/{hp.Maximum}");
            }
            else
            {
                sb.AppendLine("Hit Points: (not set)");
            }

            sb.AppendLine();
            sb.AppendLine("Skill Proficiencies:");
            if (pc.SkillProficiencies.Any())
            {
                foreach (var prof in pc.SkillProficiencies)
                {
                    var tag = prof.HasExpertise ? " (Expertise)" : prof.IsProficient ? " (Proficient)" : string.Empty;
                    sb.AppendLine($"  {prof.Skill}{tag}");
                }
            }
            else
            {
            ...
            if (pc.Resources.Any())
            {
                foreach (var res in pc.Resources)
                {
                    sb.AppendLine($"  {res.Key} ({res.Type}): {res.Current}/{res.Maximum}");
                }
            }
            else
            {
        ...
        public static string StatBlock(ICombatant combatant)
        {
            var sb = new StringBuilder();
            sb.AppendLine($"{combatant.Name} (Level {combatant.Level})");
            sb.AppendLine();

            sb.AppendLine("Ability Scores:");
            foreach (var kvp in combatant.AbilityScores.AsReadOnly())
            {
                var score = kvp.Value;
                sb.AppendLine($"  {score.Type}: {score.Score} (mod {score.Modifier:+#;-#;0})");
            }
            ...
            {
                sb.AppendLine();
                sb.AppendLine($"HP: {hp.Current}/{hp.Maximum}");
            }

            if (combatant.Conditions.Any())
            {
                sb.AppendLine();
                sb.AppendLine("Conditions:");
                foreach (var c in combatant.Conditions)
                {
                    var name = c.Type == ConditionType.Custom ? (c.CustomId ?? "Custom") : c.Type.ToString();
                    sb.AppendLine($"  {name} (Rounds remaining: {(c.RemainingRounds?.ToString() ?? "∞")})");
                }
            }
        ...
        public static string EncounterSummary(Encounter encounter)
        {
            var sb = new StringBuilder();
            ...
            sb.AppendLine();
            sb.AppendLine($"Location: {encounter.Location?.Name}");
            ...
            {
                var hp = c.Resources.FirstOrDefault(r => r.Type == ResourceType.HitPoints && r.Key == "HP");
                if (hp != null)
                    sb.AppendLine($"  {c.Name} (Lv {c.Level}) - HP {hp.Current}/{hp.Maximum}");
                else
                    sb.AppendLine($"  {c.Name} (Lv {c.Level})");
            }

            if (encounter.ActiveShatters.Any())
            {
                sb.AppendLine();
                sb.AppendLine("Active Shatters:");
                foreach (var s in encounter.ActiveShatters)
                {
